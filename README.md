...
**Architecture baseline + mobile UI prototype + 순수 구간/일자별 usage domain** 단계입니다. UI의 초기 기록과 지표는 여전히 가상 SAMPLE / DEMO DATA이며 실제 저장·계산 결과와 연결되지 않았습니다.
...
- `src/domain/usage.ts`는 누적값 정규화와 절대 instant 구간 계산을, `src/domain/dailyUsage.ts`는 명시적 meter timezone의 local-date 경계와 `actual / interpolated` provenance를 소유합니다.
...
| `src/domain/dailyUsage.ts` | 명시적 meter timezone의 일자 경계 탐색, 선형 보간, actual/interpolated provenance, 일자별 usage 분할 |
| 향후 billing-cycle domain | 검침 경계, 검침주기, 최근/주기 평균과 예측 |
...
이번 단계에 없는 기능: UI와 실제 domain 연결, 검침주기/예측/요금 계산, Google OAuth·사용자 생성·세션, Worker API, D1·migration·영속 저장, 실제 공유 권한, production service worker·push·background sync·정식 production 배포.
