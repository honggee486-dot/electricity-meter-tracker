...
- `src/persistence/d1.ts`의 모든 동적 조회값은 D1 prepared statement `?1` binding으로 전달합니다. 문자열 연결로 SQL 값을 삽입하지 않습니다.
...
root `wrangler.jsonc`는 `./dist` Static Assets와 `src/worker.ts` module entry를 함께 배포하고 `/api/*`만 Worker-first로 라우팅합니다. 정적 asset 요청은 기본 asset-first 경로를 유지합니다. **실제 D1 database/binding과 Google OAuth는 아직 없습니다.** `wrangler.local.jsonc`는 local simulation 전용이며 production/preview 배포 설정을 대체하지 않습니다. Workers Builds Git 연동과 non-production preview는 사용하지만 정식 production 릴리스는 아직 하지 않았습니다.
...
이번 단계에 없는 기능: UI와 실제 domain/API 연결, 실제 전기요금 계산, 검침 마감 설정 UI, Google OAuth·사용자 생성·세션, 실제 remote D1 database/binding, 실제 meter/readings mutation CRUD, 실제 공유 관리 UI, production service worker·push·background sync·정식 production 배포.

## Public 저장소

실제 계량기 기록, 개인정보, 로그, DB dump, 쿠키, token, secret, 실제 `.env`를 포함하지 않습니다. `persistence-tests/local_fixture.sql`은 synthetic fixture만 포함합니다. 의존성 출처·버전·integrity는 `package-lock.json`에 보존합니다. `AGENTS.md`의 작업/Git/보안 규칙을 따릅니다.
