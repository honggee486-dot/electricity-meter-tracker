...
**Architecture baseline + mobile UI prototype + 순수 usage/calendar/검침주기 domain** 단계입니다. UI의 초기 기록과 지표는 여전히 가상 SAMPLE / DEMO DATA이며 실제 저장·계산 결과와 연결되지 않았습니다.
...
- `src/domain/usage.ts`는 누적값 정규화와 절대 instant 구간 계산을, `src/domain/calendar.ts`는 IANA timezone 기반 calendar 경계를, `src/domain/dailyUsage.ts`는 일자별 분할을, `src/domain/billingCycle.ts`는 검침 마감 설정·주기·경계 보간을 소유합니다.
...
| 향후 forecast domain | 최근 일평균, 주기 평균, 마감 예상, 30일 환산, 이전 주기 비교와 신뢰도 |
...
이번 단계에 없는 기능: UI와 실제 domain 연결, 예측/요금 계산, 검침 마감 설정 UI, Google OAuth·사용자 생성·세션, Worker API, D1·migration·영속 저장, 실제 공유 권한, production service worker·push·background sync·정식 production 배포.
